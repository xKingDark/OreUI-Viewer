...
var wasmDir = location.protocol + path.dirname(location.pathname) + "/" /* "https://vgmstream.org/web/" */
...
/**
 * @overload
 * @param {unknown} dir // TODO
 * @param {string} inputFilename
 * @returns {Promise<{ inputFilename: string; outputFilename: string; wavdata: Uint8Array; }>}
 * @overload
 * @param {unknown} dir // TODO
 * @param {string} inputFilename
 * @param {ArrayBuffer} arrayBuffer
 * @returns {Promise<{ inputFilename: string; outputFilename: string; arrayBuffer: ArrayBuffer; }>}
 */
...
function getOutput(output, inputFilename, outputFilename, arrayBuffer) {
    ...
    if (!wavdata) {
        const error = new Error("vgmstream: Unsupported file");
        error.stdout = output.stdout;
        error.stderr = output.stderr;
        throw error;
    }
    ...
    // } else {
    //     output.url = URL.createObjectURL(
    //         new Blob([wavdata], {
    //             type: "audio/x-wav",
    //         })
    //     );
...
var Module = {
    ...
    wasmBinary: readFileSync(path.join(new URLSearchParams(location.search).get("dirname"), "vgmstream-cli.wasm")),
};
